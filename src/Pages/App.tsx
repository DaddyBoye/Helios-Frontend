...
interface Airdrop {
  ...
  added: boolean; // Add an added property to indicate if the airdrop is new
}
...
function App() {
  ...
  useEffect(() => {
    if (!isRemoving) {
      // Detect new airdrops that are not already visible
      const newAirdrops = airdrops.filter(airdrop =>
        !visibleAirdrops.some(visible => visible.id === airdrop.id)
      );
  
      // Add animation for new airdrops sliding in from the left
      newAirdrops.forEach(airdrop => addAirdrop(airdrop)); // Add slide-in effect
  
      // Update visibleAirdrops with new airdrops
      setVisibleAirdrops(prevAirdrops => [
        ...prevAirdrops,
        ...newAirdrops.map(airdrop => ({ ...airdrop, removing: false, added: true }))
      ]);
    }
  }, [airdrops, isRemoving, visibleAirdrops]);
  ...
  const addAirdrop = (newAirdrop: Airdrop) => {
    setVisibleAirdrops(prevAirdrops => [
      { ...newAirdrop, added: true, removing: false },
      ...prevAirdrops,
    ]);
  };
            ...
            <ul className='flex flex-col w-full items-center justify-center'>
              {visibleAirdrops.map((airdrop) => {
                const slideInAnimation = useSpring({
                  transform: airdrop.added ? 'translateX(0)' : 'translateX(-100%)',
                  opacity: airdrop.added ? 1 : 0,
                  config: { duration: 300 },
                });
                ...
                return (
                  <animated.li
                    key={airdrop.id}
                    style={slideInAnimation} // Apply slide-in animation
